build_RFSRC_DYN <- function(model, 
                            path_data_complete,
                            model_type = NULL,
                            var_sel = NULL,
                            horizon = 7,
  ...
  obj_fun <- makeSingleObjectiveFunction(
    ...
    fn =   function(x){
      ...
      inbag_file <- get_inbags_filename(subsamp_file_name_prefix_LM_30_minsize, f, x["sample.fraction"])
      load(inbag_file)
  ...
  for (f in train_files){
    ...
    if (model_type == "bin") {
      ...
      train_y <- data_train %>%
        ...
        mutate(type = if_else(type == "CLABSI", 1, 0), # make survival
               eventtime = eventtime - LM, # adjust event time from LM 
    ...
    if (model_type %in% c("bin", "multinomial")){
      ...
      train_y_no_LM <- train_y %>% select(-LM)
      data_fit <- cbind(train_X, train_y_no_LM)
    }
    ...
    inbag_file <- get_inbags_filename(subsamp_file_name_prefix_LM_30_minsize, f, subsample_size)
    load(inbag_file)
